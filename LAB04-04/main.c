...
typedef enum {stampa,ordinaD,ordinaCT,ordinaA,ordinaP,ricercaC,ricercaP,fine} e_comandi;
typedef enum {false,true} boolean;

typedef struct {
    tratta_s tratte[MAXR];
    tratta_s *tratteP[MAXR];
    int numeroTratte;
} DB;
...
DB* leggiFile(char *nomeFile){
    ...
    for (int j = 0; j <Pdatabase->numeroTratte ; j++) {
        Pdatabase->tratteP[j]=&Pdatabase->tratte[j];
    }
...
e_comandi leggiComando() {
    char riga[NR];
    fprintf(stdout,"Inserisci un comando: \n-\"stampa\" per stampare le tratte\n-\"ordinaD\" per ordinare per data\n-\"ordinaCT\" per ordinare per codice tratta\n-\"ordinaA\" per ordinare per arrivo\n-\"ordinaP\" per ordinare per partenza\n-\"ricercaC\" per ricercare una tratta tramite il codice_tratta\n\"ricercaP\" per ricercare una tratta tramite stazione di partenza\n-\"fine\" per teminare\n>:");
    fscanf(stdin, "%s", riga);
    char cmd[][NR] = {"stampa","ordinaD","ordinaCT","ordinaA","ordinaP","ricercaC","ricercaP","fine"};
...
void ordina(DB *db, e_ordinamento chiave) {
    ...
    insertionSort(db->tratteP,l,r,chiave);
}
...
void eseguiComando(DB *pDB){
    ...
    while (continua){
        switch (leggiComando()){
            case stampa: stampaTratte(pDB->tratteP,numeroTratte);  break;
            case ordinaA: printf("Elenco ordinato per stazione d'arrivo.\n"); ordina(pDB,arrivo);  break;
            case ordinaD: printf("Elenco ordinato per data.\n"); ordina(pDB,data);  break;
            case ordinaP: printf("Elenco ordinato per stazione di partenza.\n"); ordina(pDB,partenza);  break;
            case ordinaCT: printf("Elenco ordinato per codice tratta.\n"); ordina(pDB,codice_tratta); break;
            case ricercaP:{
                char part[MAXR];
                printf("Perderai l'ordinamento precedente! Le tratte verrano ordinate per stazione di partenza.\n");
                printf("Inserisci stazione di partenza:\n>: ");
                scanf("%s",part);
                ordina(pDB,partenza);
                ricercaDaPartenzaDicotomica(pDB->tratteP,numeroTratte,part,strlen(part));
            }
                break;
            case ricercaC:{
                char cod[MAXR];
                int indice;
                printf("Perderai l'ordinamento precedente! Le tratte verrano ordinate per codice_tratta.\n");
                printf("Inserisci codice tratta da cercare:\n>: ");
                scanf("%s",cod);
                indice=ricercaDaCodiceDicotomica(pDB->tratteP,numeroTratte,cod);
                indice>=0 ? printf("Elemento trovato!\nTratta [%s] da %s a %s del %s (dalle %d:%d:%d alle %d:%d:%d, con ritardo di %d minut%c)\n", pDB->tratteP[indice]->codice_tratta , pDB->tratteP[indice]->partenza, pDB->tratteP[indice]->destinazione, pDB->tratteP[indice]->dataS,pDB->tratteP[indice]->ora_arrivo.ora,pDB->tratteP[indice]->ora_arrivo.minuto,pDB->tratteP[indice]->ora_arrivo.secondi, pDB->tratteP[indice]->ora_partenza.ora,pDB->tratteP[indice]->ora_partenza.minuto,pDB->tratteP[indice]->ora_partenza.secondi, pDB->tratteP[indice]->ritardo, ((pDB->tratteP[indice]->ritardo == 1) ? 'o' : 'i')) : printf("Elemento non trovato!\n");
            }
                break;
