...
typedef enum {stampaN,stampaD,stampaCT,stampaA,stampaP,ricercaC,ricercaP,nuovoFile,fine} e_comandi;
typedef enum {false,true} boolean;

typedef struct {
    tratta_s *tratte;
    tratta_s **trattaD, **trattaCT, **trattaP, **trattaA, **trattaN;
    int numeroTratte;
} DB;
...
DB* leggiFile(char *nomeFile){
    DB *Pdatabase=(DB *) malloc(sizeof(DB));
    ...
    Pdatabase->tratte=(tratta_s *) malloc(Pdatabase->numeroTratte* sizeof(tratta_s));
    Pdatabase->trattaCT=(tratta_s **) malloc(Pdatabase->numeroTratte* sizeof(tratta_s *));
    Pdatabase->trattaA=(tratta_s **) malloc(Pdatabase->numeroTratte* sizeof(tratta_s *));
    Pdatabase->trattaD=(tratta_s **) malloc(Pdatabase->numeroTratte* sizeof(tratta_s *));
    Pdatabase->trattaP=(tratta_s **) malloc(Pdatabase->numeroTratte* sizeof(tratta_s *));
    Pdatabase->trattaN=(tratta_s **) malloc(Pdatabase->numeroTratte* sizeof(tratta_s *));
...
void liberaDB(DB *pDB){
    for (int i = 0; i <pDB->numeroTratte ; i++) {
        free(pDB->trattaP[i]);
        free(pDB->trattaCT[i]);
        free(pDB->trattaA[i]);
        free(pDB->trattaN[i]);
...
void eseguiComando(DB *pDB){
    ...
    while (continua){
        ...
        switch (leggiComando()){
            ...
            case nuovoFile: {
                char riga[MAXR];
                printf("Inserisci nome del nuovo file (completo di .txt): ");
                scanf("%s", riga);

                pDBN=leggiFile(riga);
                nT=pDBN->numeroTratte;
                newFile=true;
            }
                break;
